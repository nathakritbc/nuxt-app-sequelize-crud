...
import { updateDialog, products, updateItemDialog } from "~/hooks/products.js";
...
const updateProduct = async () => {
  try {
    ...
    const findProductByIndex: number = products.value.findIndex(
      (item: Product) => item.p_id === updateId.value
    );
    if (findProductByIndex === -1) {
      toast.error("ไม่พบรายการสินค้าที่ต้องการแก้ไข");
      return false;
    }
    const newDataUpdate: Product = {
      ...products.value[findProductByIndex],
      ...response,
    };
    products.value[findProductByIndex] = newDataUpdate;
    clearForm();
    toast.success("เเก้ไขรายการสินค้าเรียบร้อย");
  } catch (error) {
    console.error(error);
    toast.error("เเก้ไขรายการสินค้าไม่สําเร็จ!");
  }
};
